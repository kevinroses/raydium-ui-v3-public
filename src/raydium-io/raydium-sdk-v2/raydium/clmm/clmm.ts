...
import { MakeMultiTxData, MakeTxData } from "@/raydium-io/raydium-sdk-v2/common/txTool/txTool";
import { TxVersion } from "@/raydium-io/raydium-sdk-v2/common/txTool/txType";
...
export class Clmm extends ModuleBase {
  ...
  public async createPool<T extends TxVersion>(
    ...
    // console.log('mintA', mintA,mintB)
    // console.log('initialPriceX64',initPrice, mintA.decimals, mintB.decimals)
  ...
  public async increasePositionFromLiquidity<T extends TxVersion>(
    ...
    if (!ownerTokenAccountA && !ownerTokenAccountB)
      this.logAndCreateError("cannot found target token accounts", "tokenAccounts", this.scope.account.tokenAccounts);
    const poolKeys = propPoolKeys ?? (await this.getClmmPoolKeys(poolInfo.id));
    // console.log('到这数据有没有level',poolInfo)
    const levelValue = Number(poolInfo?.level || 4)
  ...
  // 铸造方法
  public async zhuzaoInstructions<T extends TxVersion>(
    props: ZhuzaoInstructions<T>,
  ): Promise<MakeTxData<T, ManipulateLiquidityExtInfo>> {
    ...
    // console.log('合约地址',CLMM_PROGRAM_ID)

    const decreaseInsInfo = await ClmmInstrument.zhuzaoInstructions({
      // programId:1,
    ...
    // console.log('怎么样',decreaseInsInfo,this.scope.ownerPubKey.toBase58())

    let nftArr = decreaseInsInfo.nftArr // PublicKey格式nft数组
    let newNftArr = [] // 字符串格式nft数组

    for(let i=0;i<nftArr.length;i++){
      // console.log('nft数组',nftArr[i].toBase58())
      newNftArr.push(nftArr[i].toBase58())
    }

    setStorageItem('newNftArr', JSON.stringify(newNftArr))
    setStorageItem('address', this.scope.ownerPubKey.toBase58())
    // console.log(getStorageItem('address'),'缓存')

    // axios.post(
    //   `${urlConfigs.BASE_HOST2}/nft/save`,
    //   {
    //     address:this.scope.ownerPubKey.toBase58(),
    //     nfts:newNftArr
    //   },
    //   { skipError: true }
    // )
    ...
    //   // console.log('send tx to be error', e.message)
    // })
    ...
    //   instructions: decreaseInsInfo.instructions,
    // });
    // console.log('到这返回什么信息',decreaseInsInfo)
    txBuilder.addInstruction(decreaseInsInfo);
    txBuilder.addCustomComputeBudget(computeBudgetConfig);
    txBuilder.addTipInstruction(txTipConfig);
    // console.log('成功没')
  ...
  // 领取方法
  public async lingquInstructions<T extends TxVersion>(
    props: LingquInstructions<T>,
  ): Promise<MakeTxData<T, ManipulateLiquidityExtInfo>> {
    ...
    let res = await axios.get(
      `${urlConfigs.BASE_HOST2}/nft/info?address=${address}`,
      {
    ...
    if(res.data.nfts.length > 0){
      nftStr = new PublicKey(res.data.nfts[0])
    } else {
      toastSubject.next({
        title: 'NFT信息错误',
        description: '未查询到当前地址下的NFT信息',
        status: 'info'
      })
      return
    }
    ...
    const decreaseInsInfo = await ClmmInstrument.lingquInstructions({
      // programId:1,
      programId:CLMM_PROGRAM_ID,
      ownerInfo: {
        wallet: this.scope.ownerPubKey,
