...
import { ReturnTypeMakeInstructions } from "@/raydium-io/raydium-sdk-v2/raydium/type";
import { ApiV3PoolInfoConcentratedItem, ApiV3Token, ClmmKeys } from "@/raydium-io/raydium-sdk-v2/api/type";
import {
  InstructionType,
  MEMO_PROGRAM_ID,
  MEMO_PROGRAM_ID2,
  ...
  parseBigNumberish,
  SYSTEM_PROGRAM_ID,
} from "@/raydium-io/raydium-sdk-v2/common";
...
import { bool, s32, struct, u128, u64, u8 } from "@/raydium-io/raydium-sdk-v2/marshmallow";
...
const anchorDataBuf = {
  ...
  zhuzao:[73, 61, 22, 154, 159, 112, 165, 244],
  lingqu:[61, 62, 40, 35, 15, 116, 133, 13],
};
...
export class ClmmInstrument {
  ...
  > {
    const { programId, owner, mintA, mintB, ammConfigId, initialPriceX64 } = props;
  ...
  static openPositionFromBaseInstruction22(
    ...
    level: number,  // 新添加的 u8 类型的 level 字段
    exTickArrayBitmap?: PublicKey,
  ): TransactionInstruction {
    const dataLayout = struct([
      s32("tickLowerIndex"),
      ...
      u8("optionBaseFlag"),
      bool("baseFlag"),
      u8("level"),  // 添加 level 字段
    ]);


    // console.log('添加level字段',level)
  ...
  static increasePositionFromLiquidityInstruction(
    ...
    // console.log('level字段',level,programId)
  ...
  // 铸造方法参数打包
  static async  zhuzaoInstruction(
    programId: PublicKey,
    payer: PublicKey,
    nftConfig: PublicKey,
    nft_user_state: PublicKey,
    ...
    const keys = [
      { pubkey: payer, isSigner: true, isWritable: true },
      { pubkey: nftConfig, isSigner: false, isWritable: true },
      { pubkey: nft_user_state, isSigner: false, isWritable: true },
      { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
    ...
    const aData = Buffer.from([...anchorDataBuf.zhuzao, ...data]);

    // console.log('打包的参数',aData)
  ...
  // 铸造方法
  static async zhuzaoInstructions({
    programId,
    ownerInfo,
    number
  }: {
    ...
    const [nftConfig] = await PublicKey.findProgramAddress(
      [Buffer.from("NFT_CONFIG")],
      programId,
    )

    const [nft_user_state] = await PublicKey.findProgramAddress(
      [Buffer.from("Nft_SUPER_USER"), ownerInfo.wallet.toBuffer()],
      programId,
    )
    ...
    for (let i = 0; i < number; i++) {
      let mint_mint = new Keypair()
      let nft_pda = await getAssociatedTokenAddressSync(mint_mint.publicKey, nft_user_state, true, TOKEN_2022_PROGRAM_ID)
      nftArr2.push(
        { pubkey: mint_mint.publicKey, isSigner: true, isWritable: true },
      )
      nftArr2.push(
        { pubkey: nft_pda, isSigner: false, isWritable: true },
      )
      nftArr.push(mint_mint.publicKey)
      signers.push(mint_mint)
    }
    
    const ins =  this.zhuzaoInstruction(
        programId,
        ownerInfo.wallet,
        nftConfig,
        nft_user_state,
  ...
  // 领取方法参数打包
  static async  lingquInstruction(
    programId: PublicKey,
    payer: PublicKey,
    address:PublicKey, // 上级地址
    nftPublicKey:PublicKey // nft
  ): Promise<TransactionInstruction> {
    const dataLayout = struct([
    ]);
    const [nftConfig] = await PublicKey.findProgramAddress(
    ...
    const [nft_user_state] = await PublicKey.findProgramAddress(
      [Buffer.from("Nft_SUPER_USER"), payer.toBuffer()],
      programId,
    )
    const [nft_super_user_state] = await PublicKey.findProgramAddress(
      [Buffer.from("Nft_SUPER_USER"), address.toBuffer()],
      programId,
    )
    let nft_pda = await getAssociatedTokenAddressSync(nftPublicKey, payer, false, TOKEN_2022_PROGRAM_ID)
    let nft_super_pda = await getAssociatedTokenAddressSync(nftPublicKey, nft_super_user_state, true, TOKEN_2022_PROGRAM_ID)
    const keys = [
      { pubkey: payer, isSigner: true, isWritable: true },
      { pubkey: nftConfig, isSigner: false, isWritable: true },
      { pubkey: nftPublicKey, isSigner: false, isWritable: false },
      { pubkey: nft_super_user_state, isSigner: false, isWritable: true },
      { pubkey: nft_user_state, isSigner: false, isWritable: true },
      { pubkey: nft_pda, isSigner: false, isWritable: true },
      { pubkey: nft_super_pda, isSigner: false, isWritable: true },
    ...
    const aData = Buffer.from([...anchorDataBuf.lingqu, ...data]);
  ...
  // 领取方法
  static async lingquInstructions({
    programId,
    ownerInfo,
    address,
    nftPublicKey
    ...
    // console.log('获取传过来的参数',address,nftPublicKey)
    
    
    const ins =  this.lingquInstruction(
        programId,
        ownerInfo.wallet,
